...
int main( int argc, char *argv[])
{
	...
	n = bind( s, (struct sockaddr *)&sll, sizeof( sll));
	if (n == -1)
	{
		perror( "bind");
		exit( 3);
	...
	while (1)
	{
		...
		if (s != -1 && FD_ISSET( s, &rfds))
		{
			...
			dprintf( "about to read payload %d bytes..\n", size);
...
#else
			size = sizeof( buf);
			dprintf( "about to read payload.. (max=%d)\n", size);
			n = read( s, buf, size);
			if (n <= 0)
			{
				if (n == 0)
					printf( "read payload hangup\n");
				else
					perror( "read");
				exit( 5);
			}
			else
				dprintf( "read payload returned %d\n", n);
#endif
			the_fd = s;
			manage_eth( buf, size);
		}
